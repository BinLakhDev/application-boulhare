...
export const getEntities: ICrudGetAllAction<IAgence> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IAgence>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`)
  };
};

export const getEntity: ICrudGetAction<IAgence> = id => {
...
export const deleteEntity: ICrudDeleteAction<IAgence> = id => async dispatch => {
  const requestUrl = `${apiUrl}/${id}`;
  const result = await dispatch({
